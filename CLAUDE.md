...
Listens on `KernelEvents::REQUEST` (main request only). If `?coupon=CODE` is present, it loads the `PromotionCoupon`, runs Sylius' `PromotionCouponEligibilityChecker` against the current cart (from `sylius.context.cart`), assigns the coupon to the cart, re-runs the `OrderProcessor`, and **persists+flushes the cart** — important because a user may arrive via a coupon link before any cart exists. Flash messages use translation keys under `setono_sylius_coupon_url_application.*`. Failures throw `RuntimeException` whose message is itself a translation key. Doctrine access goes through `Setono\Doctrine\ORMTrait::getManager($cart)`.

**2. `/coupon` page — `src/Controller/Action/ApplyCouponAction.php`**
Routed by `config/routes/shop.yaml`, exposed via either `config/routes.yaml` (with `/{_locale}` prefix) or `config/routes_no_locale.yaml` — consumers pick one in their app routing config. There are two route targets pointing at the same action: `/coupon` (full page) and `/_partial/coupon` (used via sub-request, e.g. ESI/render). The action distinguishes the two via `RequestStack::getMainRequest()` and picks `shop/coupon.html.twig` vs `shop/partial/coupon.html.twig` accordingly; the flash for "already applied" only fires on the main request to avoid duplication on partials.

**3. Admin grid column + clipboard helper — declared from the bundle extension's `prepend()`**
...
- A `sylius_grid` block adds the **URL** column to `sylius_admin_promotion_coupon` (`type: twig`, template at `templates/admin/promotion_coupon/grid/field/url.html.twig`).
- A `sylius_twig_hooks` block hooks `templates/admin/promotion_coupon/_javascripts.html.twig` into `sylius_admin.promotion_coupon.index#javascripts`. The partial inlines the translation for the "Use other base URL" label and loads `public/js/redirect.js`, which injects the input next to the column header and updates the rendered URLs on `keyup`.

There is **no** `AddUrlColumnToCouponGridSubscriber` anymore — the runtime `GridDefinitionConverterEvent` approach used in 2.x was replaced by static `sylius_grid` config in 3.0.
...
- When you change a UI surface — the `/coupon` page, the partial template, the admin grid column, or the copy-to-clipboard JS in `templates/admin/promotion_coupon/_javascripts.html.twig` — verify it in a browser via the Playwright MCP. PHPUnit/PHPStan/ECS won't catch broken Twig hooks, missing JS, or layout regressions.
